function accessDataListController($rootScope, $scope, $log, wydNotifyService, accessDataService) {
    ...
    $scope.pathSummary = null; $scope.dateRanges = []; initDateRanges();
    ...
    $scope.search = function() {
        ...
        accessDataService.doSearch(searchCriteria.application, searchCriteria.frMilliSeconds, searchCriteria.toMilliSeconds);

        $scope.showSummaryView();
    };

    $scope.onDateRangeChange = function() {
        //$log.info(searchCriteria.dateRange);
        if(searchCriteria.dateRange.st) {
            searchCriteria.frString = searchCriteria.dateRange.st.format(datePattern);
            searchCriteria.toString = searchCriteria.dateRange.ed.format(datePattern);
        }
        else {
